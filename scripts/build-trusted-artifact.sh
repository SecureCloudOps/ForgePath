...
# This is the existing ForgePath demo vulnerability policy. Trivy's nonzero
# exit code fails the workflow on HIGH/CRITICAL findings or scanner errors.
mkdir -p "$trivy_cache"
mkdir -p "$oci_layout"
...
trivy image --input "$oci_layout" --cache-dir "$trivy_cache" \
  ...
  --skip-version-check --format json --output "$scan_report"
